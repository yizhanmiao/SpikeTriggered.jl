...
Compute the Schiller overlap index between two spatial receptive fields.
...
function srf_schiller_overlap_index(e1::GaussianEllipse, e2::GaussianEllipse)
    ...
    _d = sqrt(sum(abs2, (e1.center_x - e2.center_x, e1.center_y - e2.center_y)))
    return (_sigma_sum - _d) / (_sigma_sum + _d)
end

function _schiller_sigma(ei, ej)
    _phi = _schiller_phi(ei, ej)
    return ei.axis_major * ei.axis_minor / sqrt(
...
function _schiller_phi(ei, ej)
    return atan((ej.center_y - ei.center_y) / (ej.center_x - ei.center_x))
end
