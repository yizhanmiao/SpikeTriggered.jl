...
function get_trf_polarity_score(data; thresh=0.8, reverse_off=true)
    @assert haskey(data, :on) && haskey(data, :off) "trf should be a NamedTuple with fields of `on` and `off`"
    trf = data[[:on, :off]]
